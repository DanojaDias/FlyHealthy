...
class flyhealthy{
	...
	public function n($id){
		   global $pdo;
		   $query=$pdo->prepare('SELECT name from airport_info WHERE airport_id= ?');
    		 ...
    		 $sql = 'SELECT source_airport_id, destination_airport_id, distance FROM distance';
    		 	  foreach ($pdo->query($sql) as $row) {
					  ...
					  if($start==$row['source_airport_id'] && $end==$row['destination_airport_id']){
						print $row['source_airport_id'] . "\t" ;$this->n($row['source_airport_id']);
						print "\t".$row['destination_airport_id'] . "\t";$this->n($row['destination_airport_id']);
						print "\t".$row['distance'];
						}				
						  ...
						  foreach ($pdo->query($sql) as $row1) {
							if($row1['destination_airport_id']==$end && $row['destination_airport_id']==$row1['source_airport_id']){
								 print $row['source_airport_id']."\t";$this->n($row['source_airport_id']);
								 print "\t".$row1['source_airport_id']."\t";$this->n($row1['source_airport_id']);
								 print "\t".$row1['destination_airport_id']."\t";$this->n($row1['destination_airport_id']);
								 print "\t".$row1['distance']+$row['distance']."\t";
								 $this->risk($row1['source_airport_id']) ;
								 echo "<br>";
							}
						  }			
